  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        const response = await axios.get(`https://codecollab-server-jva6.onrender.com/project/${postId}`);
  ...
  useEffect(() => {
    const assignUser = async () => {
      if (currentPost._id && currentPost.userId) {
        ...
        try {
          const res = await axios.get(`https://codecollab-server-jva6.onrender.com/project/getUserById/${usersIdForPost}`);
